    ...
    try {
      ...
      if (role === 'admin') {
        ...
        if (data.role !== 'admin') {
          ...
          throw new Error('This is an admin account. Please select Admin to login as admin.');
        }
      }
    } catch (err) {
      ...
      <div style={{ maxWidth: 1200, width: '100%' }}>
        <Card style={{ width: '100%', boxShadow: '0 12px 48px 0 rgba(37,99,235,0.13), 0 2px 8px rgba(30,64,175,0.09)', padding: '3.5rem 2.8rem 2.8rem 2.8rem', borderRadius: 32, background: '#fff', position: 'relative' }}>
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: 24 }}>
            ...
            <CardHeader>
              <CardTitle style={{ fontSize: '2rem', fontWeight: 800, letterSpacing: 0.5 }}>Login to Shopez</CardTitle>
              <CardDescription style={{ color: '#6b7280', fontSize: '1.08rem', marginTop: 6 }}>Enter your credentials to access your account</CardDescription>
              <CardAction>
          ...
          <CardContent>
            <form onSubmit={handleSubmit} style={{ width: '100%' }} className="flex flex-col gap-6">
              ...
              <div className="grid gap-2" style={{ width: '100%' }}>
                <div className="flex items-center" style={{ justifyContent: 'space-between' }}>
                  ...
                  <a
                    ...
                    style={{ fontSize: '0.92rem', color: '#ff9800', opacity: 0.85 }}
                  >
              ...
              <div className="grid gap-2" style={{ width: '100%' }}>
                ...
                <select id="role" value={role} onChange={e => setRole(e.target.value)} className="ui-input" style={{ width: '100%' }}>
                  <option value="user">User</option>
                  <option value="admin">Admin</option>
                </select>
              </div>
              {error && <div className="error-message" style={{ color: '#d32f2f', fontSize: '0.98rem' }}>{error}</div>}
              <Button type="submit" className="w-full" disabled={loading} style={{ fontSize: '1.1rem', padding: '1rem 0', marginBottom: 24, width: '100%' }}>
                {loading ? 'Logging in...' : 'Login'}
              </Button>
              <Button variant="google" className="w-full" type="button" onClick={() => alert('Google login coming soon!')} style={{ fontSize: '1.1rem', padding: '1rem 0', width: '100%' }}>
